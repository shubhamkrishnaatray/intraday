...
class IndianLargeCapBacktest:
    ...
    def print_trades(self, limit=None):
        """Print all trades or limited trades"""
        if len(self.trades_df) == 0:
            print("\n⚠️ No trades to display")
            return
        
        if limit is None:
            limit = len(self.trades_df)
        
        print(f"\n📋 COMPLETE TRADE HISTORY ({len(self.trades_df)} total trades):\n")
        
        # Display in chunks for readability
        display_df = self.trades_df.copy()
...
if __name__ == "__main__":
    ...
    print("📥 You can download this file to view all trades in Excel/Sheets")
